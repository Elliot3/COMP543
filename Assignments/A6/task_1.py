...
# Define a function to update the warp state container

def update_container(container, state):

    tenth_state = container[0]
    container.pop(0)
...
(maxSeqLen, data) = addToData(maxSeqLen, data, "data/biochemistry_processed.txt", 0, 10000)
(maxSeqLen, data) = addToData(maxSeqLen, data, "data/cancerResearch_processed.txt", 1, 10000)
(maxSeqLen, data) = addToData(maxSeqLen, data, "data/jama_processed.txt", 2, 10000)
(maxSeqLen, data) = addToData(maxSeqLen, data, "data/nature_processed.txt", 3, 10000)
(maxSeqLen, data) = addToData(maxSeqLen, data, "data/plosOne_processed.txt", 4, 10000)
...
# Initialize the container for the warp states and default zero states

zero_state = tf.Variable(np.zeros((batchSize, hiddenUnits)), dtype=tf.float32)
container = []

for i in range(10):

    container.append(zero_state)
...
for timeTick in sequenceOfLetters:
    ...
    tenth_state = update_container(container, currentState)
...
# Initialize container for the learning results
learn_results = []
...
with tf.Session() as sess:
    ...
    for epoch in range(numTrainingIters):
        ...
        if epoch % 100 == 0:
            res = ("Step", epoch, "Loss", _totalLoss, "Correct", numCorrect, "out of", batchSize)

            print(res)
            learn_results.append(res)

        if epoch >= numTrainingIters - 10:
            _accLoss += _totalLoss
            _accCount += numCorrect

    res = ("Average loss for the last 10 mini-batches is", _accLoss / 10,
...
with open('results/task_1_results.txt', 'w') as f:

    for res in learn_results[-20:]:
